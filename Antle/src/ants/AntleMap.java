...
/** Holds most of the world's actual information */
public final class AntleMap {
	...
	public AntleMap(int inw, int inh, ParamSetManager inpSM) {
		pSM = inpSM;
		w = inw;
		h = inh;
			...
			for (int x = 0; x < w; x++) {
				for (int y = 0; y < h; y++) {
					...
					if (tileType.isBreakable) {
						...
						r.setPixel(x, y,
								new int[] { (int) (((float) tileColor.getRed() * b) + tileColor.getRed()) / 2,
	...
	public BufferedImage getScentImage() {
		...
		for (int x = 0; x < w; x++) {
			for (int y = 0; y < h; y++) {
				...
				// TODO Weird math. Need to clean up and make it display accurately at high values
				int[] rgb = new int[] { (hsb >> 16) & 0xFF, (hsb >> 8) & 0xFF, hsb & 0xFF };
				r.setPixel(x, y, rgb);
			}
		}
	...
	/** @return weather block was broken */
	public boolean attackBlock(int x, int y, Entity eroder) {
	...
	private void blurMap() {
		for (Scent s : scents) {
			...
			for (int i = 0; i < (w * h) / pSM.getGlobal().scentBlurSpeed.value / 2; i++) { // Cross pattern blur
																							// droplets
			...
			for (int i = 0; i < (w * h) / pSM.getGlobal().scentBlurSpeed.value / 2; i++) { // Square pattern blur
																							// droplets
